...
function login(n) {
    var t = location.href,
        i = t.indexOf("#");
    return n && i > 0 && (t = t.substr(0, i)),
        t = t + "#" + n,
        location.href = "http://passport.cnblogs" + getHostPostfix() + "/login.aspx?ReturnUrl=" + encodeURIComponent(t),
        !1
}
function logout() {
    return confirm("确认退出吗？") && (location.href = "http://passport.cnblogs" + getHostPostfix() + "/logout.aspx?ReturnUrl=" + location.href),
        !1
}
function register() {
    return location.href = "http://passport.cnblogs" + getHostPostfix() + "/register.aspx?ReturnUrl=" + location.href,
        !1
}
...
function shBushPathPrepare() {
    for (var t = arguments,
             i = [], n = 0; n < t.length; n++) i.push(t[n].replace("@", "http://common.cnblogs.com/script/sh/"));
    return i
}
function cb_CodeHighlight() {
    var n = !1,
        t = $("pre"); (t.length && $.each(t,
        function() {
...
function cb_mathjax_render() {
    if (cb_enable_mathjax) {
        ...
        n.src = "http://cnblogsmathjax.b0.upaiyun.com/2_6_1/MathJax.js?config=TeX-AMS-MML_HTMLorMML";
        t.appendChild(n)
    }
}
...
function voteComment(n, t, i) {
    ...
    return $(i).html($(i).html().replace(/\d+/g,
        function(n) {
            return parseInt(n) + 1
        })),
        $.ajax({
...
function OpenImageUploadWindow() {
    var n = "http://upload.cnblogs" + location.hostname.substring(location.hostname.lastIndexOf(".")) + "/imageuploader/upload?host=www.cnblogs.com&editor=0#tbCommentBody";
...
function showCopyCode(n) {
    if ($(n).height() > 120) {
        var t = '<div class="cnblogs_code_toolbar"><span class="cnblogs_code_copy"><a href="javascript:void(0);" onclick="copyCnblogsCode(this)" title="复制代码"><img src="http://common.cnblogs.com/images/copycode.gif" alt="复制代码"/><\/a><\/span>';
...
function loadEncoderJs() {
    var n = document.createElement("script"),
        t;
    n.type = "text/javascript";
    n.src = "http://common.cnblogs.com/script/encoder.js";
...
function getCnblogsCodeText(n) {
    ...
    return t = t.replace(/\n(\s*\d+\s)/ig, "\n"),
        t = t.replace(/\r\n/g, "\n"),
        t = t.replace(/\nView Code/g, ""),
    typeof Encoder != undefined && (t = Encoder.htmlDecode(t)),
        $.trim(t)
}
...
function google_go_enter(n) {
    if (n.keyCode == 13) return google_go(),
        !1
}
function c_follow() {
    if (isLogined || login(), c_has_follwed) return alert("你已经关注过该博主！"),
        !1;
    if (!confirm("您确定要关注该博主吗？关注之后，就可以方便地看到他的博客了:)")) return ! 1;
    ...
    return $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        success: function(n) {
            n == "未登录" ? location.href = cnblogs.UserManager.GetLoginUrl() : (show_follow_msg(n), green_channel_success($("#green_channel_follow"), n))
        },
        error: function(n) {
            n.status > 0 && (show_follow_msg("抱歉！发生了错误！麻烦反馈至contact@cnblogs.com"), green_channel_success($("#green_channel_follow"), "抱歉！发生了错误！"))
        }
    }),
        !0
}
function remove_follow() {
    if (confirm("您确定要取消关注吗？")) {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            success: function(n) {
                n == "未登录" ? location.href = cnblogs.UserManager.GetLoginUrl() : show_follow_msg(n)
            },
            error: function(n) {
                n.status > 0 && show_follow_msg("抱歉！发生了错误！麻烦反馈至contact@cnblogs.com")
            }
        })
    }
}
function show_follow_msg(n) {
...
function loadPageBeginHtml() {
    currentBlogApp && $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        success: function(n) {
            n && (n.indexOf("<script") > -1 ? $.getScript("http://common.cnblogs.com/script/jquery.writeCapture-min.js",
                function() {
...
function loadBlogNews() {
    $.ajax({
        ...
        dataType: "text",
        success: function(n) {
            n && (n.indexOf("<script") < n.indexOf('<script type="text/javascript">cnblogs.UserManager.') ? $.getScript("http://common.cnblogs.com/script/jquery.writeCapture-min.js",
                function() {
...
function loadBlogDefaultCalendar() {
    if ($("#blog-calendar").length) {
        var t = "",
            ...
            n; (n = /\/archive\/(\d{4}\/\d{2}\/\d{2})\//g.exec(i)) ? t = n[1] : (n = /\/archive\/(\d{4}\/\d{2}\/\d{2})\./g.exec(i)) ? t = n[1] : (n = /\/archive\/(\d{4}\/\d{2})./g.exec(i)) && (t = n[1]);
        loadBlogCalendar(t)
    }
}
...
function blogCommentManager() {
            ...
            $.ajax({
                ...
                success: function(t) {
                    if (t) {
                        ...
                        i.html(t);
                        $("#tbCommentBody").bind("keydown",
                            function(n) {
                                commentManager.ctlEnterPost(n)
                            });
                        $("#btn_comment_submit").bind("click",
                            function() {
    ...
    this.PostNewComment = function() {
        ...
        $.ajax({
            ...
            timeout: 3e4,
            success: function(n) {
                if (n) n.IsSuccess ? (ShowCommentMsg("感谢您的回复:) 服务器端执行耗时" + n.Duration + "毫秒"), $("#tbCommentBody").val(""), $("#divCommentShow").html($("#divCommentShow").html() + n.Message)) : ShowCommentMsg(n.Message),
                    $("#btn_comment_submit").removeAttr("disabled");
                else ShowCommentMsg("抱歉！评论提交失败！请与管理员联系(contact@cnblogs.com)。"),
                    $("#btn_comment_submit").removeAttr("disabled")
            },
            error: function(n, t) {
    ...
    this.renderComments = function(i, r, u) {
        var f = !1,
            e = 0,
            o;
        typeof u != "undefined" && (e = u, f = !0); ! f && i == 0 && location.hash && /^#\d+$/g.test(location.hash) && (f = !0, e = location.hash.substring(1));
        i > 0 && (location.hash = "#!comments");
        o = {
            postId: cb_entryId,
            blogApp: currentBlogApp,
            pageIndex: i,
        ...
        $.ajax({
            ...
            cache: !1,
            success: function(i) {
                i ? ($("#post_comment_count").html(i.commentCount), i.commentCount > 0 ? ($("#blog-comments-placeholder").html(i.commentsHtml), f ? n("#" + e) : (location.hash == "#!comments" || location.hash == "#comments_pager_top") && n("#!comments"), comment_maxId = $("#comment-maxId").html(), comment_maxDate = $("#comment-maxDate").html()) : $("#blog-comments-placeholder").html("")) : $("#blog-comments-placeholder").html("");
                t();
                cb_CodeHighlight();
                cb_mathjax_render()
            },
...
function SubscribeComment() {
    return $("#<%= lnkSubscribe.ClientID %>").html("<span style='color:red'>订阅操作中...<\/span>"),
...
function CancelCommentSubscribe() {
    return $("#<%= lnkSubscribe.ClientID %>").html("<span style='color:red'>取消操作中...<\/span>"),
...
function GetQuoteComment(n) {
    ...
    $("#tbCommentBody").focus();
    $.getScript("http://common.cnblogs.com/script/encoder.js",
        function() {
...
function canShowAdsense() {
    var t = $("#cb_post_title_url"),
        n;
    return t.length && (n = t.html(), n.indexOf("破解") > -1 || n.indexOf("注册码") > -1 || n.indexOf("序列号") > -1 || n.indexOf("crack") > -1 || n.indexOf("下载") > -1) ? !1 : cb_entryId == 1489405 || cb_entryId == 1873020 ? !1 : !0
}
...
function incrementViewCount(n) {
    n && $.ajax({
        url: "http://counter.cnblogs.com/blog/post/" + n,
        type: "get",
        dataType: "script",
        cache: !0
    })
...
function getBlogPostBody(n) {
    $.ajax({
        ...
        dataType: "text",
        success: function(t) {
            t ? ($("#postlist_postbody_" + n).html(t), cb_CodeHighlight(), cb_mathjax_render(), fixPostListBodyFormat(), n > 3861237 && $.getScript("http://common.cnblogs.com/highlight/9.1.0/highlight.min.js?id=20160127",
                function() {
...
function google_analytics(n, t, i, r, u, f, e) {
    ...
    n[u] = n[u] ||
    function() { (n[u].q = n[u].q || []).push(arguments)
    };
    n[u].l = 1 * new Date;
    f = t.createElement(i);
...
function deliverAdT2() {
    try {
        $.ajax({
            url: "http://api.ad.cnblogs.com/adunits/text/T2/creative",
            type: "get",
            dataType: "html",
            cache: !1,
            success: function(n) {
...
function deliverDirectAdC1() {
    $.ajax({
        url: "http://api.ad.cnblogs.com/adunits/image/C1/creative",
        type: "get",
        dataType: "html",
        success: function(n) {
            $("#ad_c1").html(n)
...
function deliverGoogleAdC1() {
    $("#ad_c1").html('<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"><\/script>     <ins class="adsbygoogle"     style="display:inline-block;width:300px;height:250px"     data-ad-client="ca-pub-4210569241504288"     data-ad-slot="5457903683"><\/ins><script>(adsbygoogle = window.adsbygoogle || []).push({});<\/script>')
}
function deliverAdC2() {
    $.ajax({
        url: "http://api.ad.cnblogs.com/adunits/image/C2/creative",
        type: "get",
        dataType: "html",
        success: function(n) {
            $("#ad_c2").html(n)
...
var JSON, currentDelElement, currentCommentID, insertUBB, cnblogs;
JSON || (JSON = {}),
    function() {
        "use strict";
        function i(n) {
...
$.fn.extend({
    selection: function() {
        var n = "",
            ...
            i,
            r;
        return u ? (t = u.selection.createRange(), t.text.length > 0 && (n = t.text)) : (this.get(0).selectionStart || this.get(0).selectionStart == "0") && (i = this.get(0).selectionStart, r = this.get(0).selectionEnd, i != r && (n = this.get(0).value.substring(i, r))),
            $.trim(n)
    },
    parseHtml: function(n) {
        var t = this.get(0).document;
        if (t) this.get(0).focus(),
...
cnblogs.UserManager.GetFollowStatus = function(n) {
    $.ajax({
...
cnblogs.UserManager.FollowBlogger = function(n) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        success: function(n) {
            n == "未登录" ? location.href = cnblogs.UserManager.GetLoginUrl() : $("#p_b_follow").html(n)
        }
    })
};
cnblogs.UserManager.RemoveFlow = function(n) {
    $("#p_b_follow").html("提交中...").css("color", "red");
    $.ajax({
        url: "/mvc/Follow/RemoveFollow.aspx",
        data: '{"blogUserGuid":"' + n + '"}',
